...
import { getCompetitionCurrentMatches } from "../../../helpers/api";
import { GameRowWeb } from "@/components/ui/GameRowWeb";
import { matchImgBaseUrl } from "../../../helpers/constants";

export default function CompetitionPage({results}: {results?: boolean}) {
  ...
  useEffect(() => {
    ...
    const fetchMatches = async () => {
      ...
      try {
        const response = await getCompetitionCurrentMatches(id, results);
  ...
  useEffect(() => {
    ...
    const img = new Image();
    img.crossOrigin = "anonymous"; // necessário se for imagem externa
    ...
    img.onload = () => {
      ...
      function darkenRGB(r: number, g: number, b: number, percent: number) {
        const factor = 1 - percent;
        return `rgb(${Math.round(r * factor)}, ${Math.round(g * factor)}, ${Math.round(b * factor)})`;
      }

      function lightenRGB(r: number, g: number, b: number, percent: number) {
        return `rgb(${Math.round(r + (255 - r) * percent)}, 
              ${Math.round(g + (255 - g) * percent)}, 
              ${Math.round(b + (255 - b) * percent)})`;
      }

      const light = lightenRGB(r, g, b, 0.7);
      const dark = darkenRGB(r, g, b, 0.2);
      setBgColor(dark);
      setForeColor(light);
  ...
  return (
    ...
    >
      ...
      {competitionEdition && (
        ...
        >
          ...
          <div className="font-semibold text-lg items-end flex flex-col gap-2" >
            <span>
              {competitionEdition.competition?.name} – {competitionEdition.name}
            </span>
            <span>
              {phase}
            </span>
      ...
      <div className="px-4">
        ...
        {/* LISTAGEM (IGUAL AO MOBILE) */}
        {!loading &&
          !error &&
          Array.from(matchList.keys()).map((key) => (Array.from(matchList.get(key) || []).length > 0 &&
        ...
        {competitionEdition && (
          <div className=" flex flex-row items-center justify-between px-4 py-2 gap-4 backdrop-blur-md"
